"""Specific autograding test classes for reinforcement assignment.
...
class ValueIterationTest(test_classes.TestCase):
    """Test for Value Iteration."""

    def __init__(self, question, test_dict):
        """Initialize test from question and test dictionary."""
        ...
        self.nums_iterations_for_display = list(
            range(min(iterations, max_pre_iterations)))
    ...
    def remove_failure_file_if_exists(self):
        """Remove old failure details file if it exists."""
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        for n in self.nums_iterations_for_display:
            ...
            test_pass, std_out_string, file_out_string =\
                self.execute_n_iterations(grades, module_dict, solution_dict,
                                          n, check_policy)
            ...
            if not test_pass:
                ...
                self.add_message('For more details to help you debug, see '
                                 'test output file %s\n\n' %
                                 self.test_out_file)
    ...
    def execute_n_iterations(self, grades, module_dict, solution_dict, n,
                             check_policy):
        """Execute n iterations of value iteration and compare results."""
        test_pass = True
        values_pretty, q_values_pretty, actions, policy_pretty =\
            self.run_agent(module_dict, n)
        std_out_string = ''
        file_out_string = ''
        values_key = "values_k_%d" % n
        if self.compare_pretty_values(values_pretty,
                                      solution_dict[values_key]):
            file_out_string += "Values at iteration %d are correct.\n" % n
            file_out_string += ("   Student/correct solution:\n %s\n" %
                                self.pretty_value_solution_string(
                                    values_key, values_pretty))
        else:
            test_pass = False
            ...
            out_string += ("   Correct solution:\n %s\n" %
                           self.pretty_value_solution_string(
                               values_key, solution_dict[values_key]))
        ...
        for action in actions:
            ...
            if self.compare_pretty_values(
                    q_values, solution_dict[q_values_key]):
                file_out_string += ("Q-Values at iteration %d for action %s "
                                    "are correct.\n") % (n, action)
                file_out_string += ("   Student/correct solution:\n %s\n" %
                                    self.pretty_value_solution_string(
                                        q_values_key, q_values))
            else:
                test_pass = False
                out_string = ("Q-Values at iteration %d for action %s are "
                              "NOT correct.\n") % (n, action)
                out_string += ("   Student solution:\n %s\n" %
                               self.pretty_value_solution_string(
                                   q_values_key, q_values))
                out_string += ("   Correct solution:\n %s\n" %
                               self.pretty_value_solution_string(
                                   q_values_key, solution_dict[q_values_key]))
                std_out_string += out_string
                file_out_string += out_string
        if check_policy:
            if not self.compare_pretty_values(
                    policy_pretty, solution_dict['policy']):
                test_pass = False
    ...
    def write_solution(self, module_dict, file_path):
        ...
        with open(file_path, 'w') as handle:
            ...
            for n in self.nums_iterations_for_display:
                values_pretty, q_values_pretty, actions, policy_pretty =\
                    self.run_agent(module_dict, n)
                handle.write(self.pretty_value_solution_string('values_k_%d' %
                             n, values_pretty))
                for action in actions:
                    handle.write(self.pretty_value_solution_string(
                        'q_values_k_%d_action_%s' % (n, action),
                        q_values_pretty[action]))
            handle.write(self.pretty_value_solution_string(
                'policy', policy_pretty))
            handle.write(self.pretty_value_solution_string(
                'actions', '\n'.join(actions) + '\n'))
    ...
    def run_agent(self, module_dict, num_iterations):
        """Run the student agent for num_iterations."""
        agent = module_dict['value_iteration_agents'].ValueIterationAgent(
            self.grid, discount=self.discount, iterations=num_iterations)
        states = self.grid.get_states()
        actions = list(reduce(
                    lambda a, b: set(a).union(b),
                    [self.grid.get_possible_actions(state) for state in states]
                    ))
        values = {}
        q_values = {}
        policy = {}
        for state in states:
            ...
            for action in actions:
                ...
                if action in possible_actions:
                    q_values[action][state] =\
                        agent.compute_q_value_from_values(state, action)
                else:
    ...
    def parse_pretty_values(self, pretty):
        """Return values contained in pretty printed string."""
        values = pretty.split()
        return values


class ApproximateQLearningTest(test_classes.TestCase):
    """Test for approximate q-learning."""

    def __init__(self, question, test_dict):
        """Initialize from question and test dictionary."""
        super().__init__(question, test_dict)
        self.discount = float(test_dict['discount'])
        self.grid = grid_world.GridWorld(parse_grid(test_dict['grid']))
        if 'noise' in test_dict:
        ...
        self.nums_experiences_for_display = list(
            range(min(num_experiences, max_pre_experiences)))
        self.test_out_file = test_dict['test_out_file']
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        for n in self.nums_experiences_for_display:
            test_pass, std_out_string, file_out_string =\
                self.execute_n_experiences(grades, module_dict,
                                           solution_dict, n)
            failure_output_std_string += std_out_string
            failure_output_file_string += file_out_string
            if not test_pass:
                self.add_message(failure_output_std_string)
                self.add_message('For more details to help you debug, '
                                 'see test output file %s\n\n' %
                                 self.test_out_file)
                self.write_failure_file(failure_output_file_string)
                return self.test_fail(grades)
        self.remove_failure_file_if_exists()
    ...
    def execute_n_experiences(self, grades, module_dict, solution_dict, n):
        """Run an n experience long sub-test."""
        test_pass = True
        q_values_pretty, weights, actions, last_experience = self.run_agent(
            module_dict, n)
        std_out_string = ''
        file_out_string =\
            "==================== Iteration %d ====================\n" % n
        if last_experience is not None:
            file_out_string += ("Agent observed the transition (start_state = "
                                "%s, action = %s, end_state = %s, reward = %f)"
                                "\n\n" % last_experience)
    ...
    def write_solution(self, module_dict, file_path):
        ...
        with open(file_path, 'w') as handle:
            for n in self.nums_experiences_for_display:
                q_values_pretty, weights, actions, _ = self.run_agent(
                    module_dict, n)
                handle.write(
                    self.pretty_value_solution_string(
                        'weights_k_%d' %
                ...
                for action in actions:
                    handle.write(
                        self.pretty_value_solution_string(
                            ...
                            (n, action), q_values_pretty[action]))
        return True

    def run_agent(self, module_dict, num_experiences):
        """Run the agent for num_experiences."""
        agent = module_dict['q_learning_agents'].ApproximateQAgent(
            extractor=self.extractor, **self.opts)
        states = sorted([state for state in self.grid.get_states() if
                         len(self.grid.get_possible_actions(state)) > 0])
        rand_obj = FixedRandom().random
        # Choose a random start state and a random possible action from that
        # state.  Get the next state and reward from the transition function
        ...
        for i in range(num_experiences):
            ...
            (end_state, reward) = self.env.get_random_next_state(
                start_state, action, rand_obj=rand_obj)
...
class QLearningTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        failure_output_std_string = ''
        for n in self.nums_experiences_for_display:
            check_values_and_policy = (
                n == self.nums_experiences_for_display[-1])
            test_pass, std_out_string, file_out_string =\
                self.execute_n_experiences(grades, module_dict, solution_dict,
                                           n, check_values_and_policy)
            failure_output_std_string += std_out_string
            failure_output_file_string += file_out_string
            if not test_pass:
                self.add_message(failure_output_std_string)
    ...
    def execute_n_experiences(self, grades, module_dict, solution_dict, n,
        ...
        test_pass = True
        (values_pretty, q_values_pretty, actions, policy_pretty,
         last_experience) = self.run_agent(module_dict, n)
        std_out_string = ''
        file_out_string =\
            "==================== Iteration %d ====================\n" % n
        if last_experience is not None:
            file_out_string += ("Agent observed the transition (start_state "
                                "= %s, action = %s, end_state = %s, reward = "
                                "%f)\n\n\n") % last_experience
        for action in actions:
            q_values_key = 'q_values_k_%d_action_%s' % (n, action)
            q_values = q_values_pretty[action]
        ...
        if check_values_and_policy:
            if not self.compare_pretty_values(
                    values_pretty, solution_dict['values']):
                test_pass = False
    ...
    def write_solution(self, module_dict, file_path):
        ...
        with open(file_path, 'w') as handle:
            ...
            policy_pretty = ''
            for n in self.nums_experiences_for_display:
                values_pretty, q_values_pretty, actions, policy_pretty, _ =\
                    self.run_agent(module_dict, n)
                for action in actions:
                    handle.write(
                        self.pretty_value_solution_string(
    ...
    def run_agent(self, module_dict, num_experiences):
        """Run the student's agent for num_experiences."""
        agent = module_dict['q_learning_agents'].QLearningAgent(**self.opts)
        states = sorted([state for state in self.grid.get_states() if len(
            self.grid.get_possible_actions(state)) > 0])
        rand_obj = FixedRandom().random
        # Choose a random start state and a random possible action from that
        # state.  Get the next state and reward from the transition function
        last_experience = None
...
class EpsilonGreedyTest(test_classes.TestCase):
    """Test for epsilon greedy implementation."""
    ...
    def run_agent(self, module_dict):
        """Run the agent for self.num_experiences experiences."""
        agent = module_dict['q_learning_agents'].QLearningAgent(**self.opts)
        states = sorted([state for state in self.grid.get_states() if len(
            self.grid.get_possible_actions(state)) > 0])
        rand_obj = FixedRandom().random
    ...
    def test_epsilon_greedy(self, module_dict, tolerance=0.025):
        ...
        for state in self.grid.get_states():
            ...
            for iteration in range(self.num_iterations):
                # assume that their compute_action_from_q_values implementation
                # is correct (q4 tests this)
                if agent.get_action(state) == optimal_action:
                    num_greedy_choices += 1
            # e = epsilon, g = # greedy actions, n = num_iterations,
            # k = num_legal_actions
            # g = n * [(1-e) + e/k] -> e = (n - g) / (n - n/k)
            empirical_epsilon_numerator = (
                self.num_iterations - num_greedy_choices)
            empirical_epsilon_denominator = (
                self.num_iterations -
                self.num_iterations / float(num_legal_actions))
            empirical_epsilon = (empirical_epsilon_numerator /
                                 empirical_epsilon_denominator)
            error = abs(empirical_epsilon - self.epsilon)
            if error > tolerance:
                self.add_message(
                    "Epsilon-greedy action selection is not correct.")
                self.add_message(("Actual epsilon = %f; student empirical "
                                  "epsilon = %f; error = %f > tolerance = %f")
                                 % (self.epsilon, empirical_epsilon, error,
                                    tolerance))
                return False
        return True
...
class Question6Test(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        student_solution = str(
            student_solution).strip().lower().encode('utf-8')
...
class EvalAgentTest(test_classes.TestCase):
    """Test for Average wins of a pacman agent (q7)."""
    ...
    def __init__(self, question, test_dict):
        ...
        self.wins_minimum = (int(test_dict['wins_minimum']) if 'wins_minimum'
                             in test_dict else None)
        ...
        self.base_points = int(test_dict.get('base_points', 1))

        self.max_points = sum([len(t) for t in
                               [self.score_thresholds,
                                self.non_timeout_thresholds,
                                self.wins_thresholds]]) * self.base_points
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        games = pacman.run_games(
            **pacman.read_command(self.pacman_params.split(' ')))
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            if not passed:
                ...
                self.add_message(
                    "%s %s (%s of %s points)" %
                    (value, name, points, len(thresholds) * self.base_points))
            ...
            if minimum is not None:
                ...
                for idx, threshold in enumerate(thresholds):
                    self.add_message(
                        "    >= %s:  %s points" %
                        (threshold, (idx + 1) * self.base_points))
...
def parse_grid(string):
    """Make a gridworld grid from string representation."""
...
def compute_policy(module_dict, grid, discount):
    """Compute the policy for the given grid and discount factor."""
    value_iterator = module_dict['value_iteration_agents'].ValueIterationAgent(
...
class GridPolicyTest(test_classes.TestCase):
    """Test for proper policy in gridworld."""
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if self.question2:
            ...
            try:
                ...
                self.add_message('Did not return a (discount, noise) pair; '
                                 'instead analysis.%s returned: %s' %
                                 (self.parameter_fn, result))
                return self.test_fail(grades)
            if discount != 0.9 and noise != 0.2:
                self.add_message('Must change either the discount or the '
                                 'noise, not both. Returned (discount, noise) '
                                 '= %s' % (result, ))
                return self.test_fail(grades)
        else:
            try:
                ...
                self.add_message('Did not return a (discount, noise, living '
                                 'reward) triple; instead analysis.'
                                 '%s returned: %s' %
                                 (self.parameter_fn, result))
                return self.test_fail(grades)
        ...
        for x in range(width):
            for y in range(height):
                if self.policy[x][y] in action_map and policy[(
                        x, y)] != action_map[self.policy[x][y]]:
                    differ_point = (x, y)
                    policy_passed = False
        ...
        if not policy_passed:
            ...
            self.add_message(
                ...
                (differ_point, policy[differ_point]))
            self.add_message(
                '    Correct policy at %s: %s' %
                (differ_point,
                 action_map[self.policy[differ_point[0]][differ_point[1]]]))
            self.add_message('    Student policy:')
            self.print_policy(policy, False)
            self.add_message("        Legend:  N,S,E,W at states which move "
                             "north etc, X at states which exit,")
            self.add_message("                 . at states where the policy "
                             "is not defined (e.g. walls)")
            self.add_message('    Correct policy specification:')
            self.print_policy(self.policy, True)
            self.add_message("        Legend:  N,S,E,W for states in which "
                             "the student policy must move north etc,")
            self.add_message("                 _ for states where it doesn't "
                             "matter what the student policy does.")
        ...
        if self.path_visits is not None and self.path_visits not in path:
            ...
            self.add_message("        Legend:  N,S,E,W at states which move "
                             "north etc, X at states which exit,")
            self.add_message("                 . at states where policy not "
                             "defined")
            self.print_grid_world()
            return self.test_fail(grades)

        if self.path_not_visits is not None and self.path_not_visits in path:
            self.add_message(('Policy visits state %s when moving '
                              'without noise.') % (self.path_not_visits, ))
            self.add_message('    States visited: %s' % (path, ))
            self.add_message('    Student policy:')
            self.print_policy(policy, False)
            self.add_message("        Legend:  N,S,E,W at states which move "
                             "north etc, X at states which exit,")
            self.add_message("                 . at states where policy "
                             "not defined")
            self.print_grid_world()
            return self.test_fail(grades)
    ...
    def print_grid_world(self):
        ...
        self.add_message('        Legend: # wall, _ empty, S start, numbers '
                         'terminal states with that reward.')

    def print_policy(self, policy, policy_type_is_grid):
        """Print the grid world policy."""
        ...
        for ybar in range(self.grid.grid.height):
            y = self.grid.grid.height - 1 - ybar
            if policy_type_is_grid:
                self.add_message("        %s" % ("    ".join(
                    [legend[policy[x][y]] for x in
