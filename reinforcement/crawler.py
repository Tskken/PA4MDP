"""Class definitions for crawling robot.
...
class CrawlingRobotEnvironment(environment.Environment):
    """Environment for crawling robots."""

    def __init__(self, crawling_robot):
        """Create environemtn given a robot."""
        ...
        min_arm_angle, max_arm_angle =\
            self.crawling_robot.get_min_and_max_arm_angles()
        min_hand_angle, max_hand_angle =\
            self.crawling_robot.get_min_and_max_hand_angles()
        arm_increment = (max_arm_angle - min_arm_angle) / \
            (self.n_arm_states - 1)
        hand_increment = (max_hand_angle - min_hand_angle) / \
            (self.n_hand_states - 1)
    ...
    def get_possible_actions(self, state):
        """Return possible actions for the states in the current state."""
        actions = list()
    ...
    def do_action(self, action):
        """Perform the action and update the current state of the Environment.

        Return next_state and the reward.
        """
    ...
    def reset(self):
        ...
        Initialize the state to be the middle value for each parameter,
        e.g. if there are 13 and 19 buckets for the arm and hand parameters,
        then the intial state should be (6,9)

        Also call self.crawling_robot.set_angles() to the initial arm and
        hand angle.
        """
        arm_state = self.n_arm_states // 2
        hand_state = self.n_hand_states // 2
        self.state = arm_state, hand_state
        self.crawling_robot.set_angles(
            self.arm_buckets[arm_state], self.hand_buckets[hand_state])
        self.crawling_robot.positions = [
            20, self.crawling_robot.get_robot_position()[0]]


class CrawlingRobot:
    """Class defining a crawling robot."""
    ...
    def get_angles(self):
        """Return the pair of (arm_angle, hand_angle)."""
        return self.arm_angle, self.hand_angle

    def get_robot_position(self):
        """Return the (x,y) coordinates of the robot's lower-left point."""
    ...
    def move_hand(self, new_hand_angle):
        ...
        disp = self.displacement(
            self.arm_angle, self.hand_angle, self.arm_angle, new_hand_angle)
        cur_x_pos = self.robot_pos[0]
        self.robot_pos = (cur_x_pos + disp, self.robot_pos[1])
        ...
        # Position and Velocity Sign Post
        self.positions.append(self.get_robot_position()[0])
        # self.angle_sums.append(abs(math.degrees(old_hand_angle)-
        #                            math.degrees(new_hand_angle)))
        if len(self.positions) > 100:
            self.positions.pop(0)
            # self.angle_sums.pop(0)

    def get_min_and_max_arm_angles(self):
        """Get the lower- and upper- bound for the arm angles.
    ...
    def get_min_and_max_hand_angles(self):
        """Get the lower- and upper- bound for the hand angles.
    ...
    def get_rotation_angle(self):
        ...
        x = self.arm_length * arm_cos + self.hand_length * hand_cos\
            + self.robot_width
        y = self.arm_length * arm_sin + self.hand_length * hand_sin\
            + self.robot_height
        if y < 0:
    ...
    def displacement(self, old_arm_degree, old_hand_degree, arm_degree,
                     hand_degree):
        """Calculate displaecement between old position and new position."""
        ...
        x_old = self.arm_length * old_arm_cos + \
            self.hand_length * old_hand_cos + self.robot_width
        y_old = self.arm_length * old_arm_sin + \
            self.hand_length * old_hand_sin + self.robot_height
        ...
        if y < 0:
            if y_old <= 0:
                return (math.sqrt(x_old * x_old + y_old * y_old) -
                        math.sqrt(x * x + y * y))
            return ((x_old - y_old * (x - x_old) / (y - y_old)) -
                    math.sqrt(x * x + y * y))
        else:
            if y_old >= 0:
                return 0.0
            return (-(x - y * (x_old - x) / (y_old - y)) +
                    math.sqrt(x_old * x_old + y_old * y_old))
    ...
    def draw(self, step_count, step_delay):
        ...
        arm_cos, arm_sin = self.__get_cos_and_sin(
            rotation_angle + self.arm_angle)
        x_arm = x4 + self.arm_length * arm_cos
        y_arm = y4 - self.arm_length * arm_sin

        self.canvas.coords(self.robot_arm, x4, y4, x_arm, y_arm)

        hand_cos, hand_sin = self.__get_cos_and_sin(
            self.hand_angle + rotation_angle)
    ...
    def __init__(self, canvas):
        ...
        self.ground = canvas.create_rectangle(0,
                                              self.ground_y, self.tot_width,
                                              self.tot_height, fill='blue')

        # Robot Body #
        self.robot_width = 80
        self.robot_height = 40
        self.robot_pos = (20, self.ground_y)
        self.robot_body = canvas.create_polygon(0, 0, 0, 0, 0, 0, 0, 0,
                                                fill='green')
...
def main():
    """Run the display."""
    from graphics_crawler_display import run
    run()
